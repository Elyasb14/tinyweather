...
fn handle_client(stream: net.Stream, allocator: std.mem.Allocator) !void {
    defer {
        std.log.debug("Stream closed: {any}\n", .{stream});
        stream.close();
    }
    ...
    while (true) {
        ...
        const received_packet = tcp.Packet.decode(recv_buffer[0..bytes_read]) catch |err| {
            std.log.err("\x1b[31mClient wrote a bad packet, error\x1b[0m: {any}\n", .{err});
            return;
        };

        std.log.debug("\x1b[32mPacket received from stream\x1b[0m: {any}\n", .{received_packet});

        switch (received_packet.type) {
            .SensorRequest => {
                const decoded_request = try tcp.SensorRequest.decode(received_packet.data, allocator);
                std.log.debug("\x1b[32mDecoded SensorRequest packet\x1b[0m: {any}\n", .{decoded_request});
...
pub fn main() !void {
    ...
    defer server.deinit();
    std.log.debug("Server listening on {}\n", .{server_address});

    while (true) {
        const connection = try server.accept();
        std.log.debug("Connection established with: {any}\n", .{connection.address});
