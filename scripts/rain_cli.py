...
basins[0].apply(lambda basin: pd.read_csv(f"https://www.nrcs.usda.gov/Internet/WCIS/AWS_PLOTS/basinCharts/POR/PREC/assocHUCor_8/{basin}.csv").to_csv(f"data/{basin}.csv"))
...
for idx, ax in enumerate(fig.axes):
    ...
    ax.set_ylabel('WY Accumulated Precip (in.)')
    ax.set_title(f'{os.listdir("data")[idx]}'.strip(".csv"))
    ax.text(10, 80, f"PCT. Normal: {normal[0].round(2)}%")
    ax.legend()
fig.savefig('plots/rain_plot.png')

# test execution time
executionTime = (time.time() - startTime)
print(f'Execution time in seconds: {executionTime}')

os.system("open plots/rain_plot.png")
